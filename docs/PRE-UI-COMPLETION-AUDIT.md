...
Solana devnet, and TxLINE-backed deployment records; refreshed on 2026-07-19.
...
Spain–Argentina pool must consume its live `game_finalised` multiproof on devnet and the
...
| Real TxLINE proof | Verified | `deployments/devnet-spain-argentina-proof.json` records period-100 verification transaction `533ze…L6vq`; its exact four-stat vector was accepted by the TxLINE devnet oracle. |
...
| Deployed STAMP program | Verified | Program `7Xh5…uE5o` is executable; the period-100 upgrade signature is recorded in `deployments/devnet-spain-argentina-proof.json`. |
| Corrected live proof pool | Verified | Pool `zw7b…etc78`, both canonical Position PDAs, both forecasts, setup transactions, and two-test-USDT vault are verified by `deployments/devnet-spain-argentina-proof.json` plus the strengthened smoke command. |
...
- Vault: `DnoKxTE8j57EZNBezQnfLRKGGB6tsyNpAvDHVGGENtyc`
- Kickoff: `2026-07-19T19:00:00Z`
- Settlement eligibility: `2026-07-19T06:16:18Z`
- Refund deadline: `2026-07-21T06:16:18Z`
- Current state: `locked`, two entries, `2,000,000` base units in vault
...
```bash
npm run devnet:smoke -- deployments/devnet-spain-argentina-proof.json
npm run test:local
```
